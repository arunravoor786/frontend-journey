...
class WeatherWidget {
    constructor(containerId, apiKey = null) {
        ...
        this.currentCity = storage.get('weather_city') || 'London';
    ...
    setupEventListeners() {
        ...
        // Auto-save city preference
        cityInput?.addEventListener('change', (e) => {
            storage.set('weather_city', e.target.value);
        });
    }
    ...
    async searchWeather() {
        ...
        if (!city) {
            errorHandler.displayNotification('Please enter a city name', 'warning');
            return;
        }
    ...
    async loadWeather() {
        ...
        try {
            ...
            if (this.apiKey) {
                ...
                await AsyncUtils.delay(1000); // Simulate API delay
            }
            ...
            errorHandler.logError(error, 'WeatherWidget');
        } finally {
            this.isLoading = false;
    ...
    displayWeatherData(data) {
        ...
        if (elements.cityName) elements.cityName.textContent = data.name || this.currentCity;
        if (elements.temperature) elements.temperature.textContent = `${Math.round(data.main?.temp || 20)}°C`;
        if (elements.description) elements.description.textContent = data.weather?.[0]?.description || 'Clear sky';
        if (elements.feelsLike) elements.feelsLike.textContent = `${Math.round(data.main?.feels_like || 20)}°C`;
        if (elements.humidity) elements.humidity.textContent = `${data.main?.humidity || 60}%`;
        if (elements.windSpeed) elements.windSpeed.textContent = `${Math.round((data.wind?.speed || 5) * 3.6)} km/h`;
        if (elements.pressure) elements.pressure.textContent = `${data.main?.pressure || 1013} hPa`;
        if (elements.lastUpdated) elements.lastUpdated.textContent = `Last updated: ${new Date().toLocaleTimeString()}`;
        
        // Weather icon
        if (elements.weatherIcon && data.weather?.?.icon) 
            {
            elements.weatherIcon.src = `https://openweathermap.org/img/wn/${data.weather.icon}@2x.png`;
            elements.weatherIcon.alt = data.weather.description;
        }
        
        // Update background based on weather
        this.updateWeatherTheme(data.weather?.?.main || 'Clear');
    }
    ...
    getMockWeatherData() {
        ...
        // Find matching city or use first one
        return mockData.find(city => 
    ...
    setApiKey(apiKey) {
        ...
        this.loadWeather(); // Reload with real data
    }
}
